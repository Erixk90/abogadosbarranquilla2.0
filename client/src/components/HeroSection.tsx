...
type HeroSlide = "hero" | "news";

const SLIDES: HeroSlide[] = ["hero", "news"];

const HeroSection = () => {
  const { settings, publishedPosts } = useCms();
  const { hero } = settings;
  const [activeSlide, setActiveSlide] = useState<HeroSlide>("hero");
  ...
  const posts = useMemo(() => publishedPosts.slice(0, 3), [publishedPosts]);

  const goTo = useCallback((slide: HeroSlide) => {
    setActiveSlide(slide);
  }, []);

  const goNext = useCallback(() => {
    setActiveSlide((current) => SLIDES[(SLIDES.indexOf(current) + 1) % SLIDES.length]);
  }, []);

  const goPrev = useCallback(() => {
    setActiveSlide((current) => SLIDES[(SLIDES.indexOf(current) - 1 + SLIDES.length) % SLIDES.length]);
  }, []);
    ...
    >
      ...
      <div
        className={`absolute inset-0 transition-opacity duration-1000 ease-in-out ${
          activeSlide === "hero" ? "opacity-100" : "pointer-events-none opacity-0"
        }`}
        role="group"
        aria-roledescription="slide"
        aria-label="Presentación"
        aria-hidden={activeSlide !== "hero"}
      >
        <div className={`absolute inset-0 overflow-hidden ${activeSlide === "hero" ? "kenburns" : ""}`}>
          {hero.video ? (
            <video
              ref={videoRef}
              src={hero.video}
        ...
        <div className="relative z-10 flex min-h-screen items-center">
          <div className="container mx-auto px-4">
            <div className={`mx-auto max-w-4xl text-center text-primary-foreground ${activeSlide === "hero" ? "slide-content-in" : ""}`}>
      ...
      {/* ===== Slide 2: Resumen de noticias ===== */}
      <div
        className={`absolute inset-0 bg-primary transition-opacity duration-1000 ease-in-out ${
          activeSlide === "news" ? "opacity-100" : "pointer-events-none opacity-0"
        }`}
        role="group"
        aria-roledescription="slide"
        aria-label="Noticias recientes"
        aria-hidden={activeSlide !== "news"}
      >
        ...
        <div className="relative z-10 flex min-h-screen items-center">
          <div className="container mx-auto px-4">
            <div className={`mx-auto max-w-6xl ${activeSlide === "news" ? "slide-content-in" : ""}`}>
                    ...
                    >
                      <Card className="h-full overflow-hidden rounded-3xl border border-white/15 bg-white/10 text-primary-foreground shadow-elegant backdrop-blur transition-all duration-300 group-hover:-translate-y-1 group-hover:bg-white/15 group-hover:shadow-[0_24px_60px_-20px_hsl(var(--accent)/0.4)]">
                        ...
                        <CardContent className="space-y-3 p-5">
                          ...
                          <h3 className="min-h-14 text-lg font-semibold leading-snug text-primary-foreground">
                            {post.title}
                          </h3>
          ...
          {slides.map((slide) => (
            <button
              ...
              className={`h-2.5 rounded-full transition-all duration-500 ${
                activeSlide === slide ? "w-8 bg-accent" : "w-2.5 bg-white/50 hover:bg-white/80"
              }`}
            />
          ))}
        </div>
      ...
      >
        <div key={activeSlide} className="slide-progress h-full w-full" />
      </div>
